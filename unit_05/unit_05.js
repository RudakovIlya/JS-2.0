...
function f6() {
    const iq = document.querySelector('.i-61').value;

    const iq2 = document.querySelector('.i-62').value;
...
function f15() {
    ...
    out.textContent = Object.values(a15)
...
function f16() {
    ...
    out.textContent = Object.values(a16)
        .map((item) => item.name)
        .join(' ');
}
...
function f17() {
    ...
    out.textContent = Object.values(a17)
        .map((item) => item.age)
        .find((item) => item > 30);
}
...
function f18() {
    ...
    out.textContent = a18.hasOwnProperty(iq) ? a18[iq].join(' ') : 0;
}
